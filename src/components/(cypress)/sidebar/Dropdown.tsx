...
const Dropdown: React.FC<DropdownProps> = ({
  ...
  const { state, dispatch, workspaceId, folderId }: any = useAppState();
  ...
  const folderTitle: string | undefined = useMemo(() => {
    if (listType === "folder") {
      const stateTitle = state.workspaces
        ...
        ?.folders.find((folder: any) => folder.id === id)?.title;
  ...
  const fileTitle: string | undefined = useMemo(() => {
    if (listType === "file") {
      ...
      const stateTitle = state.workspaces
        .find((workspace: any) => workspace.id === workspaceId)
        ?.folders.find((folder: any) => folder.id === fileAndFolderId[0])
  ...
  return (
    ...
    >
      ...
      <AccordionContent>
        {state.workspaces
          .find((workspace: any) => workspace.id === workspaceId)
          ?.folders.find((folder: any) => folder.id === id)
          ?.files.filter((file: any) => !file.inTrash)
          .map((file: any) => {

...
// import { Card, CardHeader, CardTitle, CardContent } from '@/components/plura/ui/card'
...
const FunnelForm: React.FC<CreateFunnelProps> = ({
  ...
  const onSubmit = async (values: z.infer<typeof CreateFunnelFormSchema>) => {
    ...
    setTimeout(() => {
      console.log('Funnel data saved:', values);
      toast({
        title: 'Success',
        description: 'Saved funnel details',
      });
      setIsLoading(false);
      router.refresh(); // Refresh the router to show the updated data
    }, 1000);
  };

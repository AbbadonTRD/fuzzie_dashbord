...
type Props = {
  defaultOpen?: boolean
  sidebarOpt: { id: string; link: string; name: string; icon: string }[] // Adjusted to basic object structure
  sidebarLogo: string
  details: { name: string; address: string } // Simplified details type
}
...
const MenuOptions = ({
  ...
  if (!isMounted) return null // Use null for component unmounting
          ...
          <nav className="relative">
            <Command className="rounded-lg overflow-visible bg-transparent">
              ...
              <CommandList className="py-4 overflow-visible">
                ...
                <CommandGroup className="overflow-visible">
                  {sidebarOpt.map((sidebarOptions) => {
                    const iconComponent = icons.find(icon => icon.value === sidebarOptions.icon)?.path;
                    return (

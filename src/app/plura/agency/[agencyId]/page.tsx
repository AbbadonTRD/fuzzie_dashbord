...
import { Progress } from '@/components/plura/ui/progress'
import { Separator } from '@/components/plura/ui/separator'
import { Contact2, DollarSign, Goal, Link } from 'lucide-react'
import React from 'react'

// Dummy data for testing
const dummyAgencyDetails = {
  id: 'dummyAgencyId',
  connectAccountId: 'dummyConnectAccountId',
  goal: 10,
};
const dummySessions = [
  { created: '2023-01-01', amount_total: 10000, status: 'complete' },
  { created: '2023-02-01', amount_total: 20000, status: 'open' },
];
const dummySubaccounts = [{ id: 1 }, { id: 2 }];
const currency = 'USD';
const currentYear = new Date().getFullYear();

const Page = ({ params }: { params: { agencyId: string } }) => {
  const agencyDetails = dummyAgencyDetails;
  const subaccounts = dummySubaccounts;
  ...
  if (agencyDetails.connectAccountId) {
    const totalClosedSessions = dummySessions.filter(session => session.status === 'complete');
    const totalPendingSessions = dummySessions.filter(session => session.status === 'open');

    net = totalClosedSessions.reduce((total, session) => total + (session.amount_total || 0), 0) / 100;
    potentialIncome = totalPendingSessions.reduce((total, session) => total + (session.amount_total || 0), 0) / 100;

    closingRate = (totalClosedSessions.length / dummySessions.length) * 100;
  }
        ...
        <div className="absolute -top-10 -left-10 right-0 bottom-0 z-30 flex items-center justify-center backdrop-blur-md bg-background/50">
          <Card>
            <CardHeader>
              ...
              {/* Launch Pad Link can be changed accordingly */}
              <Link
                href={`/agency/${agencyDetails.id}/launchpad`}
                className="p-2 w-fit bg-secondary text-white rounded-md flex items-center gap-2"
              >
      ...
      <div className="flex flex-col gap-4 pb-6">
        <div className="flex gap-4 flex-col xl:!flex-row">
          ...
          <Card className="flex-1 relative">
            <CardHeader>
              <CardDescription>Potential Income</CardDescription>
              <CardTitle className="text-4xl">{potentialIncome ? `${currency} ${potentialIncome.toFixed(2)}` : `$0.00`}</CardTitle>
              <small className="text-xs text-muted-foreground">For the year {currentYear}</small>
            </CardHeader>
          ...
          <Card className="flex-1 relative">
            ...
            <CardContent className="text-sm text-muted-foreground">Reflects the number of sub accounts you own and manage.</CardContent>
            <Contact2 className="absolute right-4 top-4 text-muted-foreground" />
          </Card>
          <Card className="flex-1 relative">
            <CardHeader>
        ...
        {/* More cards and charts can be added here */}
      </div>
    </div>
  )
}

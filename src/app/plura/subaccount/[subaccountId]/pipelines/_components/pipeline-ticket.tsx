...
import TagComponent from "@/components/plura/global/tag";
...
import {
  ...
  AvatarFallback,
  AvatarImage,
} from "@/components/plura/ui/avatar";
import {
  Card,
  CardDescription,
  CardFooter,
...
import {
  ...
  HoverCardContent,
  HoverCardTrigger,
} from "@/components/plura/ui/hover-card";
import { toast } from "@/components/plura/ui/use-toast";
...
const PipelineTicket = ({
  ...
  const editNewTicket = (updatedTicket: TicketWithTags[0]) => {
  ...
  const handleDeleteTicket = async () => {
    try {
      ...
      // Assuming deleteTicket is a function that deletes the ticket
      const response = await deleteTicket(ticket.id);
      toast({
        title: "Deleted",
        description: "Ticket removed from lane.",
      });

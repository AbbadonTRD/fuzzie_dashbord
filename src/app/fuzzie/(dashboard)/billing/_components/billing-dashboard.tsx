...
const BillingDashboard = (props: Props) => {
  const { credits, tier } = useBilling()
  const [stripeProducts, setStripeProducts] = useState<any[]>(dummyProducts)
  const [loading, setLoading] = useState<boolean>(false)

  // Simulate loading state
  useEffect(() => {
    setLoading(false) // Set loading to false immediately since we're using dummy data
  }, [])

  const onPayment = (id: string) => {
    console.log(`Payment initiated for product ID: ${id}`);
    // Simulate redirection or payment handling
  }
